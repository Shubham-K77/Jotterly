...
const ZenDen = () => {
    ...
    const getData = async () => {
      try {
        ...
        const response = await axios.get(
          `https://jotterly-api.vercel.app/api/v1/notes/categoryNotes/ZenDen/${user._id}`,
  ...
  const searchTag = async (searchText) => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://jotterly-api.vercel.app/api/v1/notes/searchTags/${user._id}/${searchText}/ZenDen`,
        { withCredentials: true }
      );
      enqueueSnackbar(response?.data?.message, { variant: "success" });
  ...
  return (
    ...
    >
      ...
      {/* Categories */}
      <Categories categoryCount={categoryCount} category={"ZenDen"} />
      {/* Banner Image */}
      <Banner
        image={`/Images/wellnessBanner.jpg`}
        desc={
          "ZenDen – Cultivate calm and clarity with a dedicated space for wellness, mindfulness, and self-care, bringing harmony to your mind and body."
        }
      />
