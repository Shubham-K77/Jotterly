...
const WorkFlow = () => {
    ...
    const getData = async () => {
      try {
        ...
        const response = await axios.get(
          `https://jotterly-api.vercel.app/api/v1/notes/categoryNotes/WorkFlow/${user._id}`,
  ...
  const searchTag = async (searchText) => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://jotterly-api.vercel.app/api/v1/notes/searchTags/${user._id}/${searchText}/WorkFlow`,
        { withCredentials: true }
      );
      enqueueSnackbar(response?.data?.message, { variant: "success" });
