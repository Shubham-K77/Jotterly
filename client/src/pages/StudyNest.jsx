...
const StudyNest = () => {
    ...
    const getData = async () => {
      try {
        ...
        const response = await axios.get(
          `https://jotterly-api.vercel.app/api/v1/notes/categoryNotes/StudyNest/${user._id}`,
  ...
  const searchTag = async (searchText) => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://jotterly-api.vercel.app/api/v1/notes/searchTags/${user._id}/${searchText}/StudyNest`,
        { withCredentials: true }
      );
      enqueueSnackbar(response?.data?.message, { variant: "success" });
  ...
  return (
    ...
    >
      ...
      {/* Categories */}
      <Categories categoryCount={categoryCount} category={"StudyNest"} />
      {/* Banner Image */}
      <Banner
        image={`/Images/studyBanner.jpg`}
        desc={
          "StudyNest – A focused haven for study materials, notes, and research, designed to make learning seamless, organized, and stress-free."
        }
      />
